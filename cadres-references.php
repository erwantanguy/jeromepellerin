    ...
    <?php
        $terms = get_terms( ['taxonomy' => 'tagexpo','hide_empty' => 0]);
        $numero_archive = [];
        ...
        $i = 0;
        foreach ( $numero_archive as $number => $term )if ($i < 1200){
            $catid = $term[catid];
        ...
        <?php }else{?>
        <article class="col-md-3 col-sm-4 col-xs-6 item" rel="<?php echo $struture.' - '.$catid.' - '.$number; ?>">
...
?>
        ...
        <?php if($post2){?>
        <?php $os = get_field('strutures', $term);
        //print_r(in_array($post2, $os));
        //var_dump(array_search($post2, $os));
        if(in_array($post2, $os)) {?>
        <article class="col-md-3 col-sm-6 item" rel="<?php the_field('strutures', $term);echo ' - '.$catid; ?>">
	<?php echo '<a href="'.esc_url( $category_link ).'">';?>
	<?php //var_dump($term); ?>
        ...
        <?php }?>
        <?php }
        //if(in_array($post2, $os, true)){echo "Il n'y a pas encore de référence ";}?>
